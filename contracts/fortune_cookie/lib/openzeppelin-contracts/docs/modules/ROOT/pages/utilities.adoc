...
For an onchain Merkle Tree, see the xref:api:utils.adoc#MerkleTree[`MerkleTree`] library.
...
- xref:api:utils.adoc#MerkleTree[`MerkleTree`]: An onchain https://wikipedia.org/wiki/Merkle_Tree[Merkle Tree] with helper functions.
...
Building an onchain Merkle Tree allows developers to keep track of the history of roots in a decentralized manner. For these cases, the xref:api:utils.adoc#MerkleTree[`MerkleTree`] includes a predefined structure with functions to manipulate the tree (e.g. pushing values or resetting the tree).
