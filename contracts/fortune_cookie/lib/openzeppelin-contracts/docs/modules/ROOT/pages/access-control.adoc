...
The base `AccessControl` contract provides role-based access control, but it does not support onchain enumeration of role members. To track which accounts hold a role, you should instead rely on the xref:api:access.adoc#AccessControl-RoleGranted-bytes32-address-address-[RoleGranted] and xref:api:access.adoc#AccessControl-RoleRevoked-bytes32-address-address-[RoleRevoked] events, which can be processed off-chain. If onchain enumeration is required, use the xref:api:access.adoc#AccessControlEnumerable#AccessControlEnumerable[AccessControlEnumerable] extension.
